# Praisel Ekpenyong  
## SOC Analyst Tier 1

Nigeria | Open to Remote & Relocation  
📧 ekpenyongpraisel@gmail.com  
🔗 LinkedIn: https://linkedin.com/in/praiselekpenyong
💻 GitHub: https://github.com/praisel-ekpenyong  

---

## Profile

Entry-level SOC Analyst with hands-on experience in security monitoring, alert triage, and incident investigation through structured SOC labs and simulations. Experienced in analyzing SIEM alerts, Windows Event Logs, and suspicious activity to determine true positives and escalation paths within SOC environments.

---

## Core SOC Skills

- SIEM monitoring and alert triage  
- Incident investigation and escalation  
- Windows Event Log analysis  
- Network traffic analysis (PCAP)  
- MITRE ATT&CK mapping  
- Vulnerability management fundamentals  
- ServiceNow incident workflows  
- Security documentation and reporting  

---

## Tools & Technologies

- **SIEM:** Splunk  
- **Operating Systems:** Windows, Linux  
- **Identity:** Active Directory fundamentals  
- **Networking:** TCP/IP, DNS, HTTP/S  
- **Vulnerability Management:** Qualys  
- **Ticketing:** ServiceNow  
- **Cloud:** Microsoft Azure (AZ-900 level)  

---

## Hands-On SOC Case Studies (Selected)

1. **Brute Force Attack Investigation**  
   Windows authentication log analysis, SIEM correlation, MITRE ATT&CK mapping  
   👉 [View case study](./case-studies/brute-force-investigation.md)

2. **Malware Execution Investigation**  
   Suspicious process execution and Windows Event Log analysis  
   👉 [View case study](./case-studies/malware-execution-investigation.md)

3. **Windows Event Logs Investigation – Finding Suspicious Activity**  
   Windows Security Event Log analysis and timeline correlation  
   👉 [View case study](./case-studies/windows-event-logs-investigation.md)

> Each case study follows a SOC-style investigation format: alert context, evidence review, findings, conclusion, and response recommendations.

---

## Education

**B.Sc. Cybersecurity**  
Miva University  
Expected Graduation: 2027

---

## Certifications & Training

- CompTIA Security+ (Planned)  
- Splunk Core Certified User (Planned)  
- Microsoft Azure Fundamentals (AZ-900) (Planned)  
- Google Cybersecurity Professional Certificate (In Progress)  
- ServiceNow System Administrator (In Progress)  
- CyberDefenders CCD (Planned)  

---

## Objective

Focused on alert triage, log analysis, and incident response in SOC environments, with a strong interest in blue team operations and continuous learning.
